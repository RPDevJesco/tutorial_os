...
#include "../../memory/src/allocator.h"


/* =============================================================================
 * CONFIGURATION - change to higher values for smoothing
 * =============================================================================
 */
...
{
    switch (id) {
        case HAL_PLATFORM_PICO2_LAFVIN: return "Cortex-M33";
...
{
    switch (id) {
        case HAL_PLATFORM_PICO2_LAFVIN: return "ARM32";
        case HAL_PLATFORM_RPI_ZERO2W:
        case HAL_PLATFORM_RPI_3B:
...
{
    switch (id) {
        case HAL_PLATFORM_PICO2_LAFVIN: return "ARMv8-M";
        case HAL_PLATFORM_RPI_ZERO2W:
        case HAL_PLATFORM_RPI_3B:
        case HAL_PLATFORM_RPI_3BP:
        case HAL_PLATFORM_RPI_4B:
...
{
    switch (id) {
        case HAL_PLATFORM_PICO2_LAFVIN: return "SPI (ILI9488)";
        case HAL_PLATFORM_RPI_ZERO2W:
        case HAL_PLATFORM_RPI_3B:
        case HAL_PLATFORM_RPI_3BP:
...
static const char *get_boot_stage(hal_platform_id_t id, int index)
{
    switch (id) {
		...
		case HAL_PLATFORM_PICO2_LAFVIN: {
			static const char *boot_pico2[] = {
       			"ROM", "boot2", "crt0", "main()", "kernel_main", NULL
   			 };
			return boot_pico2[index];
		}
        default:
...
void kernel_main(framebuffer_t *boot_fb)
{
    ...
    /* Prime dynamic panels with first poll for First Buffer */
    fb_clear(&fb, theme.colors.bg_primary);
    draw_static_panels(&fb, &L, &theme, &s);
    dynamic_state_t d;
    ...
    /* Second buffer — now the NEW back buffer also needs static content */
    fb_clear(&fb, theme.colors.bg_primary);
    draw_static_panels(&fb, &L, &theme, &s);
    draw_dynamic_panels(&fb, &L, &theme, &s, &d);
    fb_present(&fb);
